...
export default function ProductSection({ section, flipped }: Props) {
  ...
  return (
    <section
      id={section.id}
      ref={ref}
      className="mx-auto flex max-w-6xl scroll-mt-20 flex-col justify-center gap-10 px-6 py-24 md:px-10 lg:min-h-screen lg:flex-row lg:items-center lg:gap-20 lg:py-32"
    >
      {/* Bild */}
      <div className={`lg:w-[55%] ${flipped ? "lg:order-2" : ""}`}>
        <div className={`curtain relative aspect-4/5 w-full overflow-hidden sm:aspect-3/2 lg:aspect-4/5 ${inView ? "is-open" : ""}`}>
          <Image
            src={section.image}
            alt={section.alt}
            fill
